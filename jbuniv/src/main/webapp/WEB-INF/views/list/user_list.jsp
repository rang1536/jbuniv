...
<script>
	$(document).ready(function(){
	    ...
	    ajax : {
	      ...
	      "url":"userListAjax?relationType="+relationType+"&relation1="+relation1+"&startNum="+startNum+"&type="+type ,
	      "type":"POST",
	      "dataSrc": function(json){
	    	   var list = json.list;
	    	  
	    	   $('#startNum').val(json.startNum);
	    	   $('#relationType').val(json.relationType);
	    	   $('#relation1').val(json.relation1);
	    	   
	    	   for(var i=0; i<list.length; i++){
					// 데이터 수정버튼 추가
	   	      		list[i].updateBtn ="<div align='center'><a href='modifyUser?seqNo="+list[i].seqNo+"'><input type='button' value='수정'></a></div>";		   	    			   	    	
   	           }
	    	    return json.list
	      	}
		  },            
